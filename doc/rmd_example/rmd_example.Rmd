...
knitr::kable(horses_sd_table, caption = "Standard deviation of number of horses by province")
```
...
Additionally, note that in Table \@ref(tab:horse-sd) we consider the sample standard deviation of the number of horses during the same time span as Figure \@ref(fig:horse-pops-plot).

```{r horse-plot-province-max-sd, out.width="50%", fig.cap="Horse populations for the province with the largest standard deviation"}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```


In Figure \@ref(fig:horse-plot-province-max-sd) we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
